...
**Quest 2:** left stick moves, right stick turns smoothly (continuous rotation, not a snap). Trigger = interact. Grip = grab/release. A = jump (double-tap: toggle fly). X resets the whole scene (parts, doors, switches, and your position). The right stick's forward/back axis is context-sensitive: while your right hand is holding a part, it pushes the part away or pulls it toward you (same as the desktop mouse wheel); otherwise, while walking, hold it forward to arm a teleport preview - a line and a ring show where you'd land - and release it to teleport there (let go without a valid target and nothing happens, the standard point-and-release VR comfort-locomotion gesture used to cut motion sickness); while flying, it controls altitude directly instead.
