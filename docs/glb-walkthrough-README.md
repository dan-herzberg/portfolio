...
**Quest 2:** left stick moves, right stick snap-turns. Trigger = interact. Grip = grab/release. A = jump (double-tap: toggle fly). X resets the whole scene (parts, doors, switches, and your position). While walking, hold the **right stick forward** to arm a teleport preview - a line and a ring show where you'd land - and release it to teleport there; let go without a valid target and nothing happens. This point-and-release pattern is the standard VR comfort-locomotion gesture, used specifically to cut motion sickness versus continuous joystick movement. While flying, the right stick's forward/back axis controls altitude directly instead (teleporting doesn't add much when you already have free 3D movement).
